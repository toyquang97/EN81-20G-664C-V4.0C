...
void transmitNextFloor(void)
{
	...
	txb[tib][0] = FC_3 | 3;			  // write function code + data length
	txb[tib][1] = 0x69;				  // write node id of monitorng
	txb[tib][2] = next_callfloor + 1; // write floor number
	if (next_callfloor == 255)
	{
		txb[tib][3] = 0xff;
		txb[tib][4] = 0xff; // write floor sign
	}
	else
	{
		txb[tib][3] = p.sign[next_callfloor] >> 8;
		txb[tib][4] = p.sign[next_callfloor]; // write floor sign
	}
	WriteCanB(); // transmit message and set FIFO pointer
}
